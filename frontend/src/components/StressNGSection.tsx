...
import { shortSystemName, getSystemColors, getChartTheme } from "../lib/utils";
...
export default function StressNGSection({ stressSuites, activeSystems, isDark }: Props) {
    ...
    <div className="space-y-4 animate-fade-in">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{allSuiteNames.length}개 테스트 · 5개 카테고리</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        <button
          ...
          className={`card p-3 text-left transition-all ${activeGroup === "all" ? "ring-1 ring-brand-500" : "hover:bg-slate-50 dark:hover:bg-surface-800/50"}`}
        >
          <div className="text-lg mb-1">🔥</div>
          <div className="text-xs font-semibold text-slate-800 dark:text-slate-200">전체</div>
          <div className="text-[10px] text-slate-400 dark:text-slate-500">{allSuiteNames.length}개</div>
        </button>
        {STRESS_NG_GROUPS.map((group) => {
          ...
          return (
            <button
              ...
              className={`card p-3 text-left transition-all ${isActive ? "" : "hover:bg-slate-50 dark:hover:bg-surface-800/50"}`}
              style={isActive ? { outline: `2px solid ${group.color}`, outlineOffset: "-1px" } : {}}
            >
              <div
                className="w-full h-0.5 rounded-full mb-2 transition-colors"
                style={{ backgroundColor: isActive ? group.color : isDark ? "#334155" : "#e2e8f0" }}
              />
              <div className="text-base mb-1">{group.icon}</div>
              <div className="text-[11px] font-semibold text-slate-800 dark:text-slate-200 leading-tight">{group.title}</div>
              <div className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">{count}개 테스트</div>
              {isActive && (
                <div className="mt-2 text-[9px] text-slate-500 dark:text-slate-400 leading-relaxed">
      ...
      {/* Active group HW sensitivity + scores */}
      {activeGroup !== "all" && (() => {
            ...
            <div className="flex items-start gap-4 flex-wrap">
              <div className="flex-1 min-w-[240px]">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="font-semibold text-slate-800 dark:text-slate-200">{group.titleKo}</span>
                </div>
                <div className="text-xs text-slate-400 dark:text-slate-500 mb-3">주요 하드웨어 민감도:</div>
                <ul className="space-y-1">
                  {group.hwSensitivity.map((hw, i) => (
                    <li key={i} className="text-[11px] text-slate-600 dark:text-slate-400 flex gap-2">
              ...
              <div className="flex-1 min-w-[200px]">
                <div className="text-xs text-slate-400 dark:text-slate-500 mb-2">카테고리 내 정규화 평균 점수</div>
                {sorted.map(([sys, score], i) => (
                  <div key={sys} className="flex items-center gap-2 mb-1.5">
                    <span className="text-[10px] text-slate-400 dark:text-slate-500 w-4">{i + 1}</span>
                    <div className="flex-1 bg-slate-200 dark:bg-surface-800 rounded-full h-2">
                      <div
                        className="h-2 rounded-full transition-all"
                        style={{ width: `${score * 100}%`, backgroundColor: colors[sys] }}
                      />
                    </div>
                    <span className="text-[11px] text-slate-700 dark:text-slate-300 w-28 truncate">{shortSystemName(sys)}</span>
                    <span className="text-[10px] font-mono text-slate-500 dark:text-slate-400 w-10 text-right">{(score * 100).toFixed(0)}%</span>
                  </div>
                ))}
              </div>
            </div>
...
function SuiteCard({
  ...
  const chartOption = useMemo(() => {
    ...
    if (filtered.length === 1) {
      ...
      return {
        ...
        yAxis: {
          type: "category",
          data: sorted.map((r) => shortSystemName(r.system)),
        ...
        series: [{
          ...
          label: {
            ...
            color: ct.axis.label,
            fontSize: 10,
            formatter: (p: any) => (p.value >= 1e6 ? `${(p.value/1e6).toFixed(2)}M` : p.value >= 1e3 ? `${(p.value/1e3).toFixed(1)}K` : p.value) + ` ${e.unit}`,
          },
        }],
      };
    }
    ...
    return {
      ...
      legend: {
        data: systems.map((s) => shortSystemName(s)),
      ...
      xAxis: {
        ...
        axisLabel: { color: ct.axis.faint, fontSize: 9, rotate: 30 },
      },
      yAxis: {
        type: "value",
        axisLabel: { color: ct.axis.faint, fontSize: 10 },
  ...
  }, [expanded, filtered, colors, activeSystems, isDark]);
  ...
  return (
    <div className="card overflow-hidden">
      ...
      <div className="flex items-center">
        <button onClick={onToggle} className="flex-1 flex items-center gap-3 px-4 py-3 hover:bg-slate-50 dark:hover:bg-surface-800/50 transition-colors text-left">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-sm font-medium text-slate-800 dark:text-slate-200 truncate">{suiteName}</div>
            {testInfo && (
              <div className="text-[11px] text-slate-400 dark:text-slate-500 truncate mt-0.5">{testInfo.measure}</div>
            )}
          </div>
          {/* Best system preview */}
          {filtered.length > 0 && (() => {
            ...
            return (
              <div className="shrink-0 text-right mr-3">
                ...
                <div className="text-[11px] font-mono text-green-600 dark:text-green-400 truncate max-w-[100px]">{shortSystemName(best?.system ?? "")}</div>
              </div>
            );
          })()}
          <span className="text-slate-400 dark:text-slate-600 text-xs">{expanded ? "▲" : "▼"}</span>
        </button>
        ...
        {testInfo && (
          <button
            ...
            className={`px-3 py-3 text-xs transition-colors border-l border-slate-200 dark:border-surface-700/30 ${
              guideOpen ? "text-blue-500 dark:text-blue-400 bg-blue-50 dark:bg-blue-500/10" : "text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300"
            }`}
            title="테스트 가이드 보기"
          >
            ℹ
      ...
      {/* Guide panel */}
      {guideOpen && testInfo && (
        <div className="border-t border-slate-200 dark:border-surface-700/30 bg-slate-50 dark:bg-surface-800/30 px-4 py-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-[11px]">
          {[
            ...
            <div key={label} className="space-y-1">
              <div className="text-[9px] text-slate-400 dark:text-slate-500 font-semibold uppercase tracking-wide">{label}</div>
              <div className="text-slate-700 dark:text-slate-300 leading-relaxed">{value}</div>
            </div>
          ))}
        </div>
      )}
          ...
          {(() => {
            ...
            return ranked.map((r, i) => (
              <div key={r.sys} className="flex items-center gap-1.5">
                <span className="text-[10px] text-slate-300 dark:text-slate-600">{i + 1}.</span>
                <div className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: colors[r.sys] }} />
                <span className="text-[10px] text-slate-500 dark:text-slate-400 truncate max-w-[80px]">{shortSystemName(r.sys)}</span>
              </div>
            ));
          })()}
        </div>
